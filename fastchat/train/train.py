...
class SalesCallDataset(Dataset):
    """Dataset for supervised fine-tuning on sales calls."""

    def __init__(self, raw_data, tokenizer: transformers.PreTrainedTokenizer):
        super(SalesCallDataset, self).__init__()

        rank0_print("Formatting inputs...")
        sources = [example["conversations"] for example in raw_data]
        data_dict = preprocess_call(raw_data, tokenizer)
...
def make_supervised_data_module(
    ...
    dataset_cls = (
        LazySalesCallDataset if data_args.lazy_preprocess else SalesCallDataset
    )
